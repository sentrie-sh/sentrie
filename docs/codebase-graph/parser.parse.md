...
- **Dependencies Risk:**
  ...
  - **Comments occupy statement slots.** `CommentStatement` entries are interleaved with real declarations in `Statements`, and trailing comments are lifted into their own statements. [[index.package]] and any other walker must filter them explicitly.
