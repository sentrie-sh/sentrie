...
| :--- | :--- | :--- | :--- |
| `runtime.js` | `DEPENDS_ON` | `ext.goja` | The JavaScript interpreter; every value crossing the boundary is a `goja.Value`. |
| `runtime.js` | `DEPENDS_ON` | `ext.esbuild` | Transpiles TypeScript to CommonJS JavaScript before compilation. |
| `runtime.js` | `DEPENDS_ON` | [[pack]] | `PackFile.Permissions` gates which environment variables the sandbox may see. |
| `runtime.js` | `DEPENDS_ON` | [[constants]] | `APPNAME` builds the `@sentrie/*` builtin namespace; `ExecutionStartTimeUnixKey` pins time. |
...
- **Dependencies Risk:**
  - **`require("@local/../…")` escapes the pack root.** The `@local/` branch of `resolveRequire` skips the containment check that the relative-path branch applies, enabling arbitrary file reads. Filed as a security issue; see [[runtime.js.registry]].
  ...
  - **VMs are pooled without reset**, so module-level state persists across evaluations. Filed as a separate issue; see [[runtime.modules]].
